...
public class ListaFuncionarios3 {
  ...
  private static final String caminhoArquivo = "lista-funcionarios.txt";
	...
	public static void salvarFuncionario() {		
		System.out.println("SALVANDO DADOS DE FUNCIONÁRIOS...");
		...
		try {
			...
			System.out.println("Dados de funcionários salvos!");
		}
		catch(IOException e) {
	...
	public static void lerFuncionarios() {		
		try {
			// remoção de todos os funcionários até então inseridos
			listFunc.clear();
			
			// fluxo de leitura em arquivo
			FileInputStream fluxoArquivo = new FileInputStream(caminhoArquivo);	
			// fluxo de leitura de objetos baseado em fluxo de escrita de arquivo
			ObjectInputStream fluxoObjetos = new ObjectInputStream(fluxoArquivo);

			// leitura de lista de funcionários
			...
			System.out.println("Dados de funcionários restaurados!");
		}
		catch(IOException e) {
